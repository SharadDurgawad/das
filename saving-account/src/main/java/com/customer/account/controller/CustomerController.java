...
import com.customer.account.utility.HttpUtil;
...
public class CustomerController {
	...
	@Autowired
	private BaseService<CustomerDetails> customerService;
	...
	public ResponseEntity<CustomerDetails> createCustomer(@RequestBody CustomerDetails customerDetails) {
		...
		CustomerDetails details = customerService.create(customerDetails);
		logger.debug(CommonUtil.getCallingClassAndMethodName(configuration.getEnds()));
		return new ResponseEntity<>(details, HttpUtil.getHeaders(),HttpStatus.CREATED);
	}
	...
	public ResponseEntity<CustomerDetails> getCustomer(@PathVariable String customerId) {
		logger.debug(CommonUtil.getCallingClassAndMethodName(configuration.getStarts()));
		CustomerDetails customerDetails = customerService.retrive(customerId);
		...
		logger.debug(CommonUtil.getCallingClassAndMethodName(configuration.getEnds()));
		return new ResponseEntity<>(customerDetails, HttpUtil.getHeaders(),HttpStatus.OK);
	...
	public ResponseEntity<List<CustomerDetails>> getAllCustomerDetails() {
		...
		logger.debug(CommonUtil.getCallingClassAndMethodName(configuration.getEnds()));
		return new ResponseEntity<>(customerDetailsList, HttpUtil.getHeaders(),HttpStatus.OK);
	}
	...
	public ResponseEntity<CustomerDetails> updateCustomer(@RequestBody CustomerDetails customerDetails,@PathVariable String customerId) {
		logger.debug(CommonUtil.getCallingClassAndMethodName(configuration.getStarts()));
		CustomerDetails details = customerService.update(customerDetails,customerId);
		logger.debug(CommonUtil.getCallingClassAndMethodName(configuration.getEnds()));
		return new ResponseEntity<>(details, HttpUtil.getHeaders(),HttpStatus.ACCEPTED);
	}
	...
	public ResponseEntity<CustomerDetails> removeCustomer(@PathVariable String customerId) {
		logger.debug(CommonUtil.getCallingClassAndMethodName(configuration.getStarts()));
		CustomerDetails customerDetails = customerService.remove(customerId);
		logger.debug(CommonUtil.getCallingClassAndMethodName(configuration.getEnds()));
		return new ResponseEntity<>(customerDetails, HttpUtil.getHeaders(),HttpStatus.NO_CONTENT);
	}
}
